...
@Testcontainers
class BranchControllerSecurityIT {
    ...
    private com.recruitmenterp.iam.application.service.BranchService branchService;

    @Test
    void testCreateBranch_Admin_Allowed() throws Exception {
        mockMvc.perform(post("/api/v1/branches")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"name\":\"HQ\",\"code\":\"HQ01\",\"country\":\"USA\"}")
                .with(jwt().jwt(jwt -> jwt.claim("realm_access", Map.of("roles", List.of("TENANT_ADMIN"))).claim("tenant_id", "tenant-123"))))
                .andExpect(status().isOk());
    }

    @Test
    void testCreateBranch_Recruiter_Forbidden() throws Exception {
        mockMvc.perform(post("/api/v1/branches")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"name\":\"HQ\",\"code\":\"HQ01\",\"country\":\"USA\"}")
